function HilbertEnvelope_wideBand_fir(Data,sRate,frequency_bins, run_subject, OutputPath)
...
% Esther Florin 05.03.2018
dl=length(Data);

for ff=1:length(frequency_bins)
        temp=bst_bandpass_filtfilt(Data, sRate,frequency_bins(ff,1),frequency_bins(ff,2),0,'fir'); %%iir filter ensures that low-pass has not a too large passband
        temp=(abs(hilbert((temp)')))';
    [Data_f(ff,:,:),timeout]=process_resample('Compute',temp,(0:dl-1)/sRate,1,'resample-cascade');
    clear temp
end
save([OutputPath 'HilbertEnvelope_wideBand_fir' run_subject ],'Data_f' ,'timeout','frequency_bins','-v7.3')
