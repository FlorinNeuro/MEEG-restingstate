function HilbertEnvelope_smallband_fir(Data,sRate,frequency_bins, run_subject, OutputPath)
...
for ff=1:length(frequency_bins)
    ...
    for ff_2=1:length(freqbands)-1
        ...
        temp(ff_2,:,:)=bst_bandpass_filtfilt(Data, sRate,freqbands(ff_2),freqbands(ff_2+1),0,'fir'); %%iir filter ensures that low-pass has not a too large passband
...
save([OutputPath 'HilbertEnvelope_small_band_fir' run_subject ],'Data_f' ,'timeout','frequency_bins','-v7.3')
